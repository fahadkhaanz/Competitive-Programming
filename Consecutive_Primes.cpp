...
int tc=1;
vector<ll> ans,pro;
void simpleSieve(ll int limit, vector<ll int> &prime)
{
...
int main() {
    ...
    sortall(ans);
    for(ll int i=1;i<ans.size();i++)
    {   
        cout<<ans[i]*ans[i-1]<<" ";
        pro.push_back(ans[i]*ans[i-1]);
    }
    wi(t)
    {
      solve();
